...
| `NEWSJACK_DISCORD_WEBHOOK` | Discord webhook on a channel in the server **eYKON owns** — approve on a discord draft posts there (`?wait=true`, mentions disabled). NOT the alert webhook: two variables, two audiences. Unset = discord stays copy-to-post | optional |

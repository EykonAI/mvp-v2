...
// One-shot ingestion of the GEM Global Oil Infrastructure Tracker (GOIT)
// pipelines GeoJSON (~1,800 features). Operator-supplied URL — GEM has
// no public API. Use the local script (apps/web/scripts/seed-gem-oil-pipelines.mjs)
// for the initial seed when the file is on the laptop; this endpoint is
// for any future automated quarterly refresh from a stable URL.
//
// Mirror of /api/cron/ingest-gem-gas-pipelines — only the field mapping
// differs (Countries vs CountriesOrAreas, CapacityBOEd vs CapacityBcm/y,
// StartCountry vs StartCountryOrArea).
//
...
async function handle(req: NextRequest) {
  ...
  try {
    const url = req.nextUrl.searchParams.get('url') || process.env.GEM_GOIT_PIPELINES_URL;
    if (!url) {
      return NextResponse.json({
        ok: false,
        error: 'no source URL provided',
        hint: 'pass ?url=<geojson-url> or set GEM_GOIT_PIPELINES_URL env var. The file is the GeoJSON from GEM\'s Global Oil Infrastructure Tracker download.',
      }, { status: 400 });
    }
